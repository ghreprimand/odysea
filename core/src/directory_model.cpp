...
Entry make_entry(const fs::directory_entry& element) {
    ...
    // Identity and timestamps describe the entry itself, so they come from a
    // single shared read that never follows a symbolic link.
    const detail::EntryMetadata metadata = detail::read_entry_metadata(entry.path);
    entry.identity = metadata.identity;
    entry.modified_seconds = metadata.modified_seconds;
    return entry;
}
